...
## API Integration
The `ParticipantDataSerializer` will be updated to include the `department` field. This ensures that when the frontend sends the participant data, the department is validated against the allowed choices and saved to the database.
...
- **Predefined vs. Free Text**: Using predefined choices ensures data quality but requires backend updates to add new departments. For now, sample values are sufficient as per requirements.
- **Optional vs. Required**: The field is **optional** (`null=True`, `blank=True`) to maintain backward compatibility with existing participants and to allow for cases where the department might not be known.
